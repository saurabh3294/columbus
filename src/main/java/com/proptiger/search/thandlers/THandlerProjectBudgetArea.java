...
public class THandlerProjectBudgetArea extends RootTHandler {
	...
	int[][] budgetRanges = { { 4000000, 5000000 }, { 6000000, 8000000 },
			{ 10000000, 20000000 } };
	...
	private List<Typeahead> getResultsForBudgetUnder(String templateText,
			String city) {
		List<Typeahead> results = new ArrayList<Typeahead>();

		String displayText, redirectUrl;
		for (int x : budgetPointsBelow) {
			displayText = (String.format(genericAboveBelowTextBudget,
					templateText, budgetUnit, convertBudgetAmountInWords(x),
					city));
			redirectUrl = (String.format(genericUrlCity, city.toLowerCase()) + String
					.format(genericBudgetFilter, minBudget, x));
			results.add(getTypeaheadObjectByIdTextAndURL(this.getType()
					.toString(), displayText, redirectUrl));
		}
		return results;
	}

	private List<Typeahead> getResultsForBudgetAbove(String templateText,
			String city) {
		List<Typeahead> results = new ArrayList<Typeahead>();
		...
		for (int x : budgetPointsAbove) {
			displayText = (String.format(genericAboveBelowTextBudget,
					...
					city));
			redirectUrl = (String.format(genericUrlCity, city.toLowerCase()) + String
					.format(genericBudgetFilter, x, maxBudget));
			results.add(getTypeaheadObjectByIdTextAndURL(this.getType()
					.toString(), displayText, redirectUrl));
		}
		return results;
	...
	private List<Typeahead> getResultsForBudgetBetween(String templateText,
			String city) {
		List<Typeahead> results = new ArrayList<Typeahead>();

		String displayText, redirectUrl;
		for (int[] x : budgetRanges) {
			displayText = (String.format(genericBetweenTextBudget,
					templateText, budgetUnit, convertBudgetAmountInWords(x[0]),
					convertBudgetAmountInWords(x[1]), city));
			redirectUrl = (String.format(genericUrlCity, city.toLowerCase()) + String
					.format(genericBudgetFilter, x[0], x[1]));
			results.add(getTypeaheadObjectByIdTextAndURL(this.getType()
					.toString(), displayText, redirectUrl));
		}
		return results;
	...
	private List<Typeahead> getResultsForAreaUnder(String templateText,
			String city) {
		List<Typeahead> results = new ArrayList<Typeahead>();

		String displayText, redirectUrl;
		for (int x : areaPointsBelow) {
			displayText = (String.format(genericAboveBelowTextArea,
					templateText, x, areaUnit, city));
			redirectUrl = (String.format(genericUrlCity, city.toLowerCase()) + String
					.format(genericAreaFilter, minArea, x));
			results.add(getTypeaheadObjectByIdTextAndURL(this.getType()
					.toString(), displayText, redirectUrl));
		}
		return results;
	...
	private List<Typeahead> getResultsForAreaAbove(String templateText,
			String city) {
		List<Typeahead> results = new ArrayList<Typeahead>();
		...
		for (int x : areaPointsAbove) {
			displayText = (String.format(genericAboveBelowTextArea,
					templateText, x, areaUnit, city));
			redirectUrl = (String.format(genericUrlCity, city.toLowerCase()) + String
					.format(genericAreaFilter, x, maxArea));
			results.add(getTypeaheadObjectByIdTextAndURL(this.getType()
					.toString(), displayText, redirectUrl));
		}
		return results;
	...
	private List<Typeahead> getResultsForAreaBetween(String templateText,
			String city) {
		List<Typeahead> results = new ArrayList<Typeahead>();

		String displayText, redirectUrl;
		for (int[] x : areaRanges) {
			displayText = (String.format(genericBetweenTextArea, templateText,
					x[0], x[1], areaUnit, city));
			redirectUrl = (String.format(genericUrlCity, city.toLowerCase()) + String
					.format(genericAreaFilter, x[0], x[1]));
			results.add(getTypeaheadObjectByIdTextAndURL(this.getType()
					.toString(), displayText, redirectUrl));
		}
		return results;
