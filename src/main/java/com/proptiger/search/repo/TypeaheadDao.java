...
public class TypeaheadDao {
	...
	public QueryResponse getResponseV2(String query, int rows,
			List<String> filterQueries) {
	...
	public List<Typeahead> getTypeaheadsV2(String query, int rows,
		...
		// if (!filterQueries.isEmpty()) { // Adding another query if filters
		// exist
		...
		List<Typeahead> results = getSpellCheckedResponseV2(solrQuery, rows,
				filterQueries);
	...
	private String getBoostQuery(String query) {
		...
		for (int i = 0; i < count - 1; i++) {
			boostQuery += "Core_text:" + st.nextToken() + "^"
					+ Math.max(1, boost) + " ";
	...
	private SolrQuery getSimpleSolrQuery(String query, int rows,
			List<String> filterQueries) {
	 ...
	 */
	private List<Typeahead> getSpellCheckedResponseV2(SolrQuery solrQuery,
			int rows, List<String> filterQueries) {
