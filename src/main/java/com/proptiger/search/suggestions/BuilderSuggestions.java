...
public class BuilderSuggestions {
	...
	public List<Typeahead> getSuggestionsByCity(int id, String name,
			String redirectUrl, String cityName, String cityUrlPrefix, int count) {
		List<Typeahead> suggestions = new ArrayList<Typeahead>();
		Typeahead obj;
		for (String[] template : suggestionTemplates) {
			obj = new Typeahead();
			obj.setDisplayText(String.format(template[0], name) + " in"
					+ cityName);
			obj.setRedirectUrl(cityName.toLowerCase() + "/"
					+ String.format(template[1], redirectUrl));
