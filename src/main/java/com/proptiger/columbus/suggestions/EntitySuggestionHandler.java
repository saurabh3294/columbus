...
public class EntitySuggestionHandler {
	...
	public List<Typeahead> getEntityBasedSuggestions(List<Typeahead> results,
			int count) {
		...
		if (results == null || results.isEmpty()
				|| results.get(0).getScore() < suggestionScoreThreshold) {
			return suggestions;
		}
		...
		switch (idTokens[1]) {
		case "PROJECT":
			suggestions = projectSuggestions.getSuggestions(entityId, label,
					redirectUrl, count);
			break;
		case "CITY":
			suggestions = citySuggestions.getSuggestions(entityId, label,
					redirectUrl, count);
			break;
		case "BUILDER":
			suggestions = builderSuggestions.getSuggestions(entityId, label,
					redirectUrl, count);
			break;
		case "SUBURB":
			suggestions = suburbSuggestions.getSuggestions(entityId, label,
					redirectUrl, topResult.getCity(), count);
			break;
		case "LOCALITY":
			String cityName = topResult.getCity();
			String localityName = topResult.getLocality();
			suggestions = localitySuggestions.getSuggestions(entityId, label,
					redirectUrl, cityName, localityName, count);
			break;
		case "LANDMARK":
