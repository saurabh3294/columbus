...
public class NLPSuggestionHandler {
	...
	public List<Typeahead> getNlpTemplateBasedResults(String query,
			String city, int rows) {

		List<String> queryFilters = new ArrayList<String>();
		queryFilters.add("DOCUMENT_TYPE:TYPEAHEAD" + " AND "
				+ "TYPEAHEAD_TYPE:TEMPLATE");
		QueryResponse response = typeaheadDao.getResponseV4(query, rows,
				queryFilters);
		...
		RootTHandler thandler = templateMap.getTemplate(templateHits.get(0)
				.getTemplateText().trim());

		List<Typeahead> resultsFirstHandler = new ArrayList<Typeahead>();
		if (thandler != null) {
			resultsFirstHandler = thandler.getResults(query,
					templateHits.get(0), city, rows);
		}
		...
		for (Typeahead t : templateHits) {
			...
			if (rootTaTemplate != null) {
				...
				logger.warn("No Template Handler found for typeahead template : "
						+ templateText);
			}
		}
