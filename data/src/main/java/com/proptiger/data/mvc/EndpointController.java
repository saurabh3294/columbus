...
public class EndpointController {
	...
	public ProAPIResponse show() {
		...
		Iterator<RequestMappingInfo> keyItr = handlerMethodMap.keySet()
				.iterator();
		while (keyItr.hasNext()) {
			...
			MethodParameter[] methodParameters = handlerMethod
					.getMethodParameters();
			if (methodParameters != null && methodParameters.length > 0) {
				...
				for (MethodParameter methodParameter : methodParameters) {
					...
					parameters.append(getParameterType(methodParameter))
							.append(SPACE);
					...
					if (requestParam != null) {
						...
						if (!ValueConstants.DEFAULT_NONE.equals(requestParam
								.defaultValue())) {
							required = false;
						}
						...
						if (!required) {
							api.append(SQUARE_BRACKET_START)
									.append(parameterName).append(EQUAL)
									.append(ANGULAR_BRACKET_START)
									.append(parameterName)
									.append(ANGULAR_BRACKET_END)
									.append(SQUARE_BRACKET_END);
						} else {
							api.append(parameterName).append(EQUAL)
									.append(ANGULAR_BRACKET_START)
									.append(parameterName)
									.append(ANGULAR_BRACKET_END);
						}
	...
	private void sortMethodParamsByRequired(MethodParameter[] methodParameters) {
		Collections.sort(Arrays.asList(methodParameters),
				new Comparator<MethodParameter>() {
					@Override
					public int compare(MethodParameter o1, MethodParameter o2) {
						RequestParam requestParam1 = o1
								.getParameterAnnotation(RequestParam.class);
						RequestParam requestParam2 = o2
								.getParameterAnnotation(RequestParam.class);
						if (requestParam1 != null && requestParam2 != null) {
							return new Boolean(requestParam2.required())
									.compareTo(new Boolean(requestParam1
											.required()));
						}
						return 0;
					}
	 ...
	 * @return
	 */
	private String getMethodName(StringBuilder api,
		...
		if (mappingInfo.getMethodsCondition() == null
			...
			for (RequestMethod m : mappingInfo.getMethodsCondition()
					.getMethods()) {
				method = method + m;
			}
		}
	...
	private String getParameterType(MethodParameter methodParameter) {
		...
		if (methodParameter.getParameterType().getName().contains(".")) {
			String[] list = methodParameter.getParameterType().getName()
					.split("\\.");
