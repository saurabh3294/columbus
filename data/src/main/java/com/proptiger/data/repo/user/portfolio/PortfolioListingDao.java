...
public interface PortfolioListingDao extends JpaRepository<PortfolioListing, Integer> {
    ...
    @Query("SELECT PL FROM PortfolioListing PL LEFT JOIN fetch PL.otherPrices OP LEFT JOIN fetch PL.listingPaymentPlan LPP " + " WHERE PL.userId = ?1 AND PL.sourceType IN ?2 AND PL.listingStatus IN ?3 ORDER BY PL.listingId DESC ")
    ...
    public PortfolioListing findByUserIdAndListingIdAndListingStatusIn(
            Integer userId,
            Integer listingId,
            List<ListingStatus> listingStatus);

    /**
     * This query should be used by internal service and should not be exposed
     * to users
     * 
     * @param listingId
     * @param listingStatus
    ...
    public List<PortfolioListing> findByTypeIdInAndListingStatusAndSourceTypeInAndUserIdIn(
            List<Integer> typeId,
            ListingStatus listingStatus,
            List<Source> sourceType,
            List<Integer> userId);

    public List<PortfolioListing> findByTypeIdInAndListingStatusAndSourceTypeInAndUserIdNotIn(
            List<Integer> typeId,
            ListingStatus listingStatus,
            List<Source> sourceType,
            List<Integer> userId);

    public List<PortfolioListing> findByTypeIdInAndListingStatusAndSourceTypeIn(
            List<Integer> typeId,
            ListingStatus listingStatus,
            List<Source> sourceType);
