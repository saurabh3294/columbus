...
public interface LeadTaskDao extends JpaRepository<LeadTask, Integer> {
    ...
    @Query(value = "SELECT TOLM FROM LeadTask LT INNER JOIN LT.offeredListingMappings TOLM INNER JOIN FETCH TOLM.offeredListing LOL WHERE LT.id = ?1")
    public List<TaskOfferedListingMapping> getMappedListingMappingsForTask(int taskId);

    @Query(value = "SELECT DISTINCT(TOLM) FROM TaskOfferedListingMapping TOLM join fetch TOLM.offeredListing LOL join fetch LOL.listing LI left join fetch LI.projectSupply left join fetch LI.currentListingPrice join fetch LI.property LIP join fetch LIP.project LIPP join fetch LIPP.projectStatusMaster join fetch LIPP.builder join fetch LIPP.locality LIPPL join fetch LIPPL.suburb LIPPLS join fetch LIPPLS.city where LIPP.version = 'Website' and LI.status = 'Active' and TOLM.taskId in (?1)")
    ...
    @Query("select LT from LeadTask LT JOIN FETCH LT.taskStatus LTS JOIN FETCH LTS.resultingStatus INNER JOIN FETCH LTS.masterLeadTask MLT INNER JOIN FETCH LTS.masterLeadTaskStatus MLTS LEFT JOIN FETCH LT.statusReason WHERE LT.leadOfferId in (?1)")
    public List<LeadTask> findTasksByLeadOfferId(int leadOfferId);

    @Query("select LT from LeadTask LT JOIN FETCH LT.taskStatus LTS JOIN FETCH LTS.resultingStatus INNER JOIN FETCH LTS.masterLeadTask MLT INNER JOIN FETCH LTS.masterLeadTaskStatus MLTS LEFT JOIN FETCH LT.statusReason WHERE LT.id in (?1)")
