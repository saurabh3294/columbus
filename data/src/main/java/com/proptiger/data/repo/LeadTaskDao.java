...
public interface LeadTaskDao extends JpaRepository<LeadTask, Integer> {
    ...
    @Query("select LT from LeadTask LT left join fetch LT.offeredListingMappings TOLM left join fetch TOLM.offeredListing LOL left join fetch LOL.listing LI left join fetch LI.currentListingPrice left join fetch LI.property LIP left join fetch LIP.project LIPP left join fetch LIPP.locality where (LIPP.version is null or LIPP.version = 'Website') and (LI.status  is null or LI.status = 'Active') and LT.id in (?1)")
