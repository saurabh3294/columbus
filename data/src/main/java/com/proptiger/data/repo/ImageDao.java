...
public interface ImageDao extends ImageCustomDao, JpaRepository<Image, Long> {
    @Query("select I from Image as I JOIN FETCH I.imageTypeObj IT JOIN FETCH IT.objectType O where O.type = ?1 and I.objectId IN ?2 AND I.active=1 ORDER BY IT.priority, I.priority, I.takenAt DESC, I.id DESC")
    public List<Image> getImagesForObject(String obj, long objectId);

    @Query("select I from Image as I JOIN FETCH I.imageTypeObj IT JOIN FETCH IT.objectType O where O.type = ?1 and IT.type = ?2 and I.objectId IN ?3 AND I.active=1 ORDER BY IT.priority, I.priority, I.takenAt DESC, I.id DESC")
    ...
    @Query("SELECT I FROM Image as I JOIN FETCH I.imageTypeObj IT JOIN FETCH IT.objectType O WHERE I.originalHash = ?1 AND I.objectId = ?2 AND I.imageTypeObj.objectType.type = ?3 AND I.active = 1")
