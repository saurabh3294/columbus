...
public interface LeadOfferDao extends JpaRepository<LeadOffer, Integer>, LeadOfferCustomDao {
    ...
    @Query("select LO from LeadOffer LO join fetch LO.lead L join fetch LO.masterLeadOfferStatus MLOS left join LO.nextTask NT  where LO.agentId = ?1 order by NT.scheduledFor DESC, LO.createdAt ASC")
    public List<LeadOffer> getLeadOffersForAgent(int agentId);

    @Query("select LO from LeadOffer LO join fetch LO.lead L join fetch LO.masterLeadOfferStatus MLOS left join LO.nextTask NT where LO.agentId = ?1 and NT.scheduledFor > ?2 order by NT.scheduledFor DESC, LO.createdAt ASC")
    public List<LeadOffer> getLeadOffersForAgentDueDateGreatherThan(int agentId, Date dueDate);
   
    @Query("select LO from LeadOffer LO join fetch LO.lead L join fetch LO.masterLeadOfferStatus MLOS left join LO.nextTask NT where LO.agentId = ?1 and LO.statusId in ?2 order by NT.scheduledFor DESC, LO.createdAt ASC")
    public List<LeadOffer> getLeadOffersForAgentStatusIdsIn(int agentId, List<Integer> statusIds);
   
    
    @Query("select LO from LeadOffer LO join fetch LO.lead L join fetch LO.masterLeadOfferStatus MLOS left join LO.nextTask NT where LO.agentId = ?1 and LO.statusId in ?2 and LO.nextTask.scheduledFor > ?3 order by NT.scheduledFor DESC, LO.createdAt ASC")
    public List<LeadOffer> getLeadOffersForAgentWhereStatusIdsInAndDueDateGreatherThan(int agentId,List<Integer> statusIds, Date dueDate);
