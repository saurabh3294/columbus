...
public interface LocalityDao extends PagingAndSortingRepository<Locality, Integer>, LocalityCustomDao {
    ...
    @Query("Select L.localityId, AVG(LR.overallRating) as overallAvgRating from Locality L left join L.localityRatings LR " +
            " where L.localityId = LR.localityId AND (L.cityId=?1 OR L.suburbId=?2) AND L.isActive = 1 AND (L.localityId not in (?4) or ?4 is NULL) "
