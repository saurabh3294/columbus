...
public class TypeaheadDao {
   ...
   /*If the query has a typo and can be corrected then new query is generated using the suggestions and executed
    ...
    public List<Typeahead> getTypeaheadsV2(String query, int rows, List<String> filterQueries) {
        ...
        query = this.parseCities(query, cityList);//removes city name if query contains other terms too
        ...
        if(!filterQueries.isEmpty()){//Adding another query if filters exist
        	solrQueries.add(this.getSolrQueryV2(query, rows, new ArrayList<String>()));
        }
    ...
    /*If the query has a typo and can be corrected then new query is generated using the suggestions and 
     executed automatically*/
    
    private List<Typeahead> getResponse(SolrQuery solrQuery,int rows,List<String> filterQueries){  	
