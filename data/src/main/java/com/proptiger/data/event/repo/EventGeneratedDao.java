...
public interface EventGeneratedDao extends PagingAndSortingRepository<EventGenerated, Integer> {
    ...
    @Query("Select e from EventGenerated e JOIN Fetch e.eventType et JOIN Fetch et.subscribers s where e.eventStatus = ?1 and s.subscriberName = ?2 and et.name IN ?3 and e.id > s.lastEventGeneratedId order by e.id asc")
    ...
    @Query("Select e from EventGenerated e JOIN Fetch e.eventType et JOIN Fetch et.subscribers s where e.eventStatus = ?1 and s.subscriberName = ?2 and e.id > s.lastEventGeneratedId order by e.id asc")
    public List<EventGenerated> getLatestEventGeneratedBySubscriber(
            EventStatus EventStatus,
            SubscriberName subscriberName,
            Pageable pageable);
    ...
    @Modifying
    @Query("Update EventGenerated e set e.eventStatus = ?3 WHERE e.eventTypeUniqueKey = ?2 and e.eventTypeId  = (Select et.id from EventType et WHERE et.name = ?1)")
