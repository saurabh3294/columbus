...
import com.proptiger.core.event.model.payload.EventTypeUpdateHistory;
...
import com.proptiger.core.model.event.EventType.HoldingPeriodType;
...
public abstract class DBEventProcessor implements EventProcessor {
	...
	private static Logger logger = LoggerFactory
			.getLogger(DBEventProcessor.class);
	 ...
	 * @return
	 */
	public List<EventGenerated> processRawEvents(Integer eventTypeId,
			List<EventGenerated> events) {

		// Gets the Processed events of a particular type from DB which are
		// still in holding state
		List<EventGenerated> processedEvents = eventGeneratedService
				.getProcessedEventsToBeMerged(eventTypeId);
		...
		updateEventsByOldStatusMap.put(EventStatus.Processed,
				new ArrayList<EventGenerated>());

		// TODO to process them in separate threads
		for (Map.Entry<String, List<EventGenerated>> entry : groupEventMap
				.entrySet()) {
			handleRawEventsStrategy(entry, allCurrentProcessedEvents,
					updateEventsByOldStatusMap);
		}
		...
		eventGeneratedService
				.updateEventsOnOldEventStatus(updateEventsByOldStatusMap);
		return events;
	}
	...
	public List<EventGenerated> processVerifiedEvents(
			List<EventGenerated> events) {
		return events;
	}
	...
	protected void handleProcessedEventsStrategy(
		...
		for (EventGenerated eventGenerated : finalEvents) {
			...
			EventGenerated newEventGenerated = eventGeneratedService
					.saveOrUpdateOneEvent(eventGenerated);

			// If event is marked Successfully for pending verification then
			// send it for verification
			defaultEventTypeConfig = (DefaultEventTypeConfig) eventGenerated
					.getEventType().getEventTypeConfig();
			if (newEventGenerated.getEventStatus().equals(
					EventStatus.PendingVerification)) {
				defaultEventTypeConfig.getEventVerificationObject()
						.verifyEvents(newEventGenerated);
			}
		}
	}

	protected EventGenerated suppressProcessedEvents(
			Map.Entry<String, List<EventGenerated>> entry) {
		return suppressEvents(entry.getValue());
	}

	protected EventGenerated mergeProcessedEvents(
			Map.Entry<String, List<EventGenerated>> entry) {
	...
	protected void handleRawEventsStrategy(
		...
		if (eventType.getStrategy().equals(EventType.Strategy.SUPPRESS)) {
			finalEvents.add(suppressRawEvents(entry, allCurrentProcessedEvents,
					updateEventsByOldStatusMap));
		} else if (eventType.getStrategy().equals(EventType.Strategy.MERGE)) {
			finalEvents.add(mergeRawEvents(entry, allCurrentProcessedEvents,
					updateEventsByOldStatusMap));
		} else {
			finalEvents.addAll(entry.getValue());
		}
	...
	protected EventGenerated suppressRawEvents(
		...
		List<EventGenerated> processedEventsByEventStatus = allCurrentProcessedEvents
				.get(entry.getKey());
		if (processedEventsByEventStatus != null) {
			...
			updateEventsByOldStatusMap.get(EventStatus.Processed).addAll(
					processedEventsByEventStatus);
		}
	...
	protected EventGenerated mergeRawEvents(
		...
		List<EventTypePayload> childEventTypePayloads = lastEvent
				.getEventTypePayload().getChildEventTypePayloads();
		...
		if (processedEventsByEventStatus != null) {
			for (EventGenerated eventGenerated : processedEventsByEventStatus) {
				...
				childEventTypePayloads
						.add(eventGenerated.getEventTypePayload());
			}
			updateEventsByOldStatusMap.get(EventStatus.Processed).addAll(
					processedEventsByEventStatus);
		}
	...
	protected EventGenerated suppressEvents(List<EventGenerated> events) {
		...
		for (EventGenerated eventGenerated : events) {
			if (lastEvent == null
					|| (lastEvent.getId() < eventGenerated.getId())) {
	...
	protected EventGenerated mergeEvents(List<EventGenerated> events) {
		...
		List<EventTypePayload> childPayloads = lastEvent.getEventTypePayload()
				.getChildEventTypePayloads();
		if (childPayloads == null) {
			lastEvent.getEventTypePayload().setChildEventTypePayloads(
					childEventTypePayloads);
		} else {
	 ...
	 * @return
	 */
	protected Map<String, List<EventGenerated>> groupEventsByKey(
			List<EventGenerated> events) {
		...
		for (EventGenerated eventGenerated : events) {
			eventsGeneratedByKeyGroup = groupEventsByUniqueKey
					.get(eventGenerated.getEventTypeUniqueKey());
			...
			groupEventsByUniqueKey.put(eventGenerated.getEventTypeUniqueKey(),
	 ...
	 * @param eventStatus
	 */
	protected void updateEventHistories(EventGenerated eventGenerated,
			EventStatus eventStatus) {
		List<EventTypeUpdateHistory> eventTypeUpdateHistories = eventGenerated
				.getEventTypePayload().getEventTypeUpdateHistories();
		if (eventTypeUpdateHistories == null) {
			eventTypeUpdateHistories = new ArrayList<EventTypeUpdateHistory>();
		}
		EventTypeUpdateHistory newHistory = new EventTypeUpdateHistory(
				eventStatus, new Date());
		eventTypeUpdateHistories.add(newHistory);
		eventGenerated.getEventTypePayload().setEventTypeUpdateHistories(
				eventTypeUpdateHistories);
		eventGenerated.setData(Serializer.toJson(eventGenerated
				.getEventTypePayload()));
	}
	...
	protected void updateEventExpiryTime(EventGenerated eventGenerated) {
		HoldingPeriodType holdingPeriodType = eventGenerated.getEventType()
				.getHoldingPeriodType();
		Integer holdingPeriodValue = eventGenerated.getEventType()
				.getHoldingPeriodValue();
		...
		logger.debug("EVENT TYPE ID " + eventGenerated.getId()
				...
				+ ". SETTING EXPIRY DATE: " + expiryDate);

		eventGenerated.setExpiryDate(expiryDate);
	}

	protected void updateEventStatus(EventGenerated eventGenerated,
			EventStatus eventStatus) {
