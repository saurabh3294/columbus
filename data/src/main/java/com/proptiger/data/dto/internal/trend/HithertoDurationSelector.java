...
/**
 * This class functions as a selector for duration in Hitherto-Trends.
 * The durations are mutually exclusive in a way that, any of them can be set but
 * only one of them will be valid (by current logic, its the one that 
 * was set last) i.e every set operation will override the last one.
 */
...
public class HithertoDurationSelector 
{
	...
	public static int StartMonthFinancialYear = 3;				//April (0-to-11 month numbers are  0-to-11)
	public static int StartMonthYear = 0;						//January (0-to-11 month numbers are  0-to-11)
	...
	public static String currentMonth = "0000-00-00";			// Set by TrendService
	...
	/* Following calculations make 2 assumptions :
	 * 1. A DB entry for first day of the month contains data for the whole month.
	 * 2. Trend service will query [startDay, endDay].
	 */
	...
	private void recalulateMonthRangeUsingYearFinancialYearDuration()
	{
		...
		calculateMonthRangeForYearDuration(monthsElapsedInFinYear, yearStartMonth, MonthThresholdForFinacialYear, financialYearDuration);
	}
	
	private void calculateMonthRangeForYearDuration(int monthsElapsed, String yearStartMonth, int monthThreshold, int duration)
	{
		/* If number of months passed in current year are less than the threshold. */
