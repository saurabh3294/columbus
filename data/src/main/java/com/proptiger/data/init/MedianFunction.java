...
public class MedianFunction implements SQLFunction {
    ...
    public String render(Type arg0, List arg1, SessionFactoryImplementor arg2) throws QueryException {
        if (arg1.size() != 1) {
            throw new QueryException(new IllegalArgumentException("median should have one arg"));
        }

        String medianFunction = "substring_index(substring_index(group_concat(" + arg1.get(0)
                + " order by "
                + arg1.get(0)
                + "), ',', ceil(count(*)/2)), ',', -1)";
