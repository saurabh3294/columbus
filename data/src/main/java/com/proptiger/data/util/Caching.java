...
public class Caching {
	@Cacheable(key ="#key", value =Constants.Cache.CACHE)
	public <T> T getSavedResponse(String key, Class<T> object){
		return null;
	}
	
	@CachePut(key="#key", value=Constants.Cache.CACHE)
	public <T> T saveResponse(String key, T response){
		return response;
	}
	
	@CacheEvict(key="#key", value=Constants.Cache.CACHE, beforeInvocation=true)
