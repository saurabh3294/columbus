...
public class BuilderService {
	...
	public Builder getBuilderInfo(Integer builderId, Selector selector) {
		...
		if (builder == null) {
			throw new ResourceNotAvailableException(ResourceType.BUILDER,
		 ...
		 */
		Selector totalProjectSelector = createSelectorForTotalProjectOfBuilder(
				builderId, projectStatusNotIn, selector);
		SolrServiceResponse<List<Project>> totalProjects = projectService
				.getProjects(totalProjectSelector);

		builder.setTotalProjects(((Long) totalProjects.getTotalResultCount())
				.intValue());
		/*
		...
		Selector selectorForOnGoingProject = createSelectorForTotalProjectOfBuilder(
				builderId, projectStatusNotIn, selector);
		SolrServiceResponse<List<Project>> ongoingProjects = projectService
				.getProjects(selectorForOnGoingProject);
		builder.setTotalOngoingProjects(((Long) ongoingProjects
				.getTotalResultCount()).intValue());

		Iterator<Project> totalProjectItr = totalProjects.getResult()
				.iterator();
		Iterator<Project> ongoingProjectItr = ongoingProjects.getResult()
				.iterator();
		int counter = 0;
		...
		while (ongoingProjectItr.hasNext() && counter++ < projectsToShow) {
			...
			List<Image> projectImages = imageService.getImages(
					DomainObject.project, "main", project.getProjectId());
		...
		while (totalProjectItr.hasNext() && counter++ < projectsToShow) {
			...
			List<Image> projectImages = imageService.getImages(
					DomainObject.project, null, project.getProjectId());
			if (projectImages != null && projectImages.size() > 1) {
				List<Image> list = new ArrayList<>();
				list.add(projectImages.get(0));
				project.setImages(list);
	 ...
	 * @return
	 */
	private Selector createSelectorForTotalProjectOfBuilder(Integer builderId,
			List<String> projectStatusNotIn, Selector selectorPassed) {
