...
public class PropertyService {
	...
	public PaginatedResponse<List<Project>> getPropertiesGroupedToProjects(
		...
		if (propertyListingSelector != null
				&& propertyListingSelector.getPaging() != null
			...
			for (int start = startOriginal; remainingRowsToBeFetched > 0
					&& rowsFetchedLast == ROWS_THRESHOLD; start += ROWS_THRESHOLD) {
				propertyListingSelector.getPaging().setStart(start);
				propertyListingSelector.getPaging().setRows(
						Math.min(ROWS_THRESHOLD, remainingRowsToBeFetched));
	...
	public PaginatedResponse<List<Property>> getProperties(FIQLSelector selector) {
		PaginatedResponse<List<Property>> response = propertyDao
	...
	public Map<String, Map<String, Map<String, FieldStatsInfo>>> getStatsFacetsAsMaps(
			Selector selector, List<String> fields, List<String> facet) {
		...
		for (Map.Entry<String, FieldStatsInfo> entry : stats.entrySet()) {
			...
			if (entry.getValue() == null
					|| entry.getValue().getFacets() == null)
				continue;

			for (Map.Entry<String, List<FieldStatsInfo>> e : entry.getValue()
					.getFacets().entrySet()) {
				facetName = e.getKey();
	...
	public Property getProperty(int propertyId) {
		String jsonSelector = "{\"paging\":{\"rows\":1},\"filters\":{\"and\":[{\"equal\":{\"propertyId\":"
				+ propertyId + "}}]}}";
		Selector selector = new Gson().fromJson(jsonSelector, Selector.class);

		List<Property> properties = getProperties(selector);
		if (properties == null || properties.isEmpty())
			throw new ResourceNotAvailableException(ResourceType.PROPERTY,
	 ...
	 */
	@Transactional
	public Property createUnverifiedPropertyOrGetExisting(Listing listing,
			Integer userId) {
		Property property = null;
		OtherInfo otherInfo = listing.getOtherInfo();
		if (otherInfo != null && otherInfo.getSize() > 0
				&& otherInfo.getBedrooms() > 0 && otherInfo.getProjectId() > 0) {
			FIQLSelector selector = new FIQLSelector()
					.addAndConditionToFilter(
							"projectId==" + otherInfo.getProjectId())
					.addAndConditionToFilter(
							"bedrooms==" + otherInfo.getBedrooms())
					.addAndConditionToFilter("size==" + otherInfo.getSize())
					.addAndConditionToFilter(
							"project.version==" + DataVersion.Website);

			if (otherInfo.getBathrooms() > 0) {
				selector.addAndConditionToFilter("bathrooms=="
						+ otherInfo.getBathrooms());
			}
			PaginatedResponse<List<Property>> propertyWithMatchingCriteria = getPropertiesFromDB(selector);
			if (propertyWithMatchingCriteria != null
					&& propertyWithMatchingCriteria.getResults() != null
				...
				selector = new FIQLSelector()
						...
						.addAndConditionToFilter(
								"projectId==" + otherInfo.getProjectId())
						.setRows(1).addSortDESC("countPropertyId");

				propertyWithMatchingCriteria = getPropertiesFromDB(selector);
				Property toCreate = Property.createUnverifiedProperty(userId,
						otherInfo, propertyWithMatchingCriteria.getResults()
								.get(0).getUnitType());
	 ...
	 * @return
	 */
	public PaginatedResponse<List<Property>> getPropertiesFromDB(
			FIQLSelector selector) {
		PaginatedResponse<List<Property>> paginatedResponse = new PaginatedResponse<List<Property>>();
		EntityManager entityManager = emf.createEntityManager();
		AbstractQueryBuilder<Property> builder = new JPAQueryBuilder<>(
				emf.createEntityManager(), Property.class);
	...
	public void setCouponCatalogueForProperties(List<Property> properties) {
		...
		Map<Integer, CouponCatalogue> map = getCoupCatalogueService()
				.getCouponCatalogueMapByPropertyIds(propertyIds);
	...
	public CouponCatalogueService getCoupCatalogueService() {
		if (couponCatalogueService == null) {
			couponCatalogueService = applicationContext
					.getBean(CouponCatalogueService.class);
		}
	...
	public Property getActivePropertyByIdFromDB(int propertyId) {
		...
		fiqlSelector.addAndConditionToFilter("optionCategory=="
				+ EntityType.Actual + ";(unitType==" + UnitType.Apartment
				+ ",unitType==" + UnitType.Villa + ",unitType=="
				+ UnitType.Plot + ")" + ";propertyId==" + propertyId);
		PaginatedResponse<List<Property>> properties = getPropertiesFromDB(fiqlSelector);

		if (properties == null || properties.getResults() == null
				|| properties.getResults().isEmpty())
			return null;
