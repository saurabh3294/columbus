...
public class RecommendationService {
    ...
    /**
     * This method will get all properties of a project. Then get the similar
     * properties of each property of the project. Now taking all project ids of
     * all similar properties found and sorting them and return n number of
     * similar projects based on limit.
     * 
     * @param projectId
     * @param limit
    ...
    public List<Project> getSimilarProjects(int projectId, int limit) {
        ...
        for (SolrResult projectProperty : properties) {
            ...
            /*
             * data = getSimilarProperties(propertyId, limit); if(data == null)
             * continue;
             */
        ...
        if (similarProjectIds.size() > 0) {
            List<SolrResult> solrResults = projectDao.sortingSimilarProjects(
                    similarProjectIds,
                    latitude,
                    longitude,
                    assignedPriority,
    ...
    /**
     * This method will get the similar properties of a property Id. It will
     * sort the similar properties. If the viewed property has non valid
     * lat/long and no localityId then isPropertyNearBy flag will be false.
     * 
     * @param propertyId
     *            viewed property
     * @param limit
     *            number of similar properties to return.
     * @return Map<String, Object> It contains: 1: propertyData:
     *         List<SolrResult> similar properties found. 2: isPropertyNearBy:
     *         boolean whether similar properties found are in nearbyrange.
     */
    public List<Property> getSimilarProperties(long propertyId, int limit) {
        // distance, budget%, area%, sort Priority
        int[][] params = new int[][] { { 5, 15, 15, 1 }, { 5, 30, 15, 2 }, { 10, 15, 15, 3 }, { 10, 30, 15, 4 } };
    ...
    /**
     * This method will take the viewedProperty Object and return the number of
     * similar properties found. Based on the params, it will iterate through
     * all params until it finds the limit number of similar properties. These
     * params will define the filtering criteria for finding the similar
     * property. Then assign the priority to each property based on lat/long,
     * price, size, status.
     * 
    ...
    private List<List<SolrResult>> getSimilarPropertiesData(SolrResult viewPropertyData, int limit, int[][] params) {
        ...
        for (int i = 0; i < params.length && totalProperties < limit; i++) {
            ...
            tempSearchProperties = propertyDao.getSimilarProperties(
                    params[i][0],
                    latitude,
                    longitude,
                    minArea,
    ...
    /**
     * This method will assign priority by moving the properties from high
     * priority list to low priority list. The list with index lower has high
     * priority.
     * 
    ...
    private void assignPriorityToProperty(List<List<SolrResult>> searchPropertiesData, SolrResult viewedPropertyData) {
        ...
        while (listIt.hasNext()) {
            ...
            while (it.hasNext()) {
                ...
                // area and location data both is false but either of them is
                // present
                ...
                // if both budget, lat long is not present and property do not
                // belong to same locality
    ...
    /**
     * This method will take the project status of the property and return the
    ...
    /**
     * This method will return the project status grouping for the similar
     * properties.
     * 
    ...
    /**
     * This method will check the data of property to get the validity of
     * similar property. It will check the following conditions: 1: whether area
     * is null or not. Price is null or not. 2: where it has valid lat/long or
     * localityId
     * 
     * @param propertyData
     * @param projectStatusGroup
     * @return boolean array 0 => similar properties Data valid 1 => area or
     *         location data is present. 2 => area and location data both is
     *         present.
     */
    private boolean[] isSimilarPropertySearchValid(SolrResult propertyData, List<Object> projectStatusGroup) {
        ...
        Boolean locationStatus = (!checkDoubleObject(latitude) && !checkDoubleObject(longitude)) || checkDoubleObject(localityId
                .doubleValue());
    ...
    private List<SolrResult> sortProperties(List<List<SolrResult>> searchPropertiesData, final SolrResult viewedProperty) {
        ...
        PropertyComparer propertyComparer = new PropertyComparer(
                ...
                viewedProperty.getProperty().getBudget());
    ...
    /**
     * This method will determine whether similar properties found was in the
     * property near by area or not.
     * 
     * @param solrResult
    ...
    private boolean isPropertySearchedNearBy(SolrResult solrResult) {
        ...
        return !checkDoubleObject(solrResult.getProject().getLatitude()) && !checkDoubleObject(solrResult.getProject()
                .getLongitude());
    }
}
